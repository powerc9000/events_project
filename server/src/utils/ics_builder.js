...
ICSFileBuilder.prototype.toString = function() {
  ...
  if (this.calname)
    lines.push("X-WR-CALNAME:" + this.calname.toString().trim());
  if (this.method) lines.push("METHOD:" + this.method.toString().trim());
  if (this.prodid) lines.push("PRODID:" + this.prodid.toString().trim());
  if (this.timezone)
    lines.push("X-WR-TIMEZONE:" + this.timezone.toString().trim());
  if (this.tzid) {
    var tzData;
    try {
      tzData = require(path.join(
        __dirname,
        "../timezones/database",
        this.tzid
          .toString()
    ...
    for (var key in tzData.VTIMEZONE) {
      if (tzData.VTIMEZONE.hasOwnProperty(key)) {
        if (
          typeof tzData.VTIMEZONE[key] == "string" ||
          tzData.VTIMEZONE[key] instanceof String
        ) {
  ...
  for (var idx = 0; idx < this.events.length; idx++) {
    ...
    if (_event.allDay) {
      ...
      lines.push(
        "DTSTART:" + _formatDate(_event.start, false, _event.floating)
      );
    ...
    if (_event.description)
      lines.push("DESCRIPTION:" + _escape(_event.description));
    ...
    if (_event.organizer) {
      ...
      if (_event.organizer.sentBy) {
        settings.push(`SENT-BY="MAILTO:${_event.organizer.sentBy}"`);
      }
      settings.push(`CN="${_event.organizer.name.replace(/"/g, '\\"')}"`);
      lines.push(`${settings.join(";")}:MAILTO:${_event.organizer.email}`);
    }
    for (var ac = 0; ac < _event.attendees.length; ac++) {
      if (_event.attendees[ac] instanceof Error) {
        ...
        lines.push(
          "ATTENDEE;ROLE=" +
            _event.attendees[ac].role +
            ";PARTSTAT=" +
            _event.attendees[ac].status +
            (_event.attendees[ac].rsvp ? ";RSVP=TRUE" : "") +
            ";CN=" +
            _event.attendees[ac].name +
            ":MAILTO:" +
            _event.attendees[ac].email
        );
      }
    }
    ...
    for (var additionalProp in _event.additionalTags) {
      if (_event.additionalTags.hasOwnProperty(additionalProp)) {
        lines.push(
          additionalProp + ":" + _event.additionalTags[additionalProp]
        );
      }
    }
...
ICSFileBuilder.prototype._checkAndBuildEventObject = function(rawEvent) {
  ...
  if (rawEvent.repeating) {
    ...
    if (
      !rawEvent.repeating.freq ||
      allowedRepeatingFreq.indexOf(rawEvent.repeating.freq.toUpperCase()) === -1
    ) {
      return new Error(
        "event.repeating.freq is a mandatory item, and must be one of the following: " +
          allowedRepeatingFreq.join(", ")
      );
    }
  ...
  (rawEvent.attendees || []).forEach(function(att) {
    if (att.name && att.email) {
      ...
      event.attendees.push(
        new Error(
          "Invalid attendee data, name and email is required: " +
            JSON.stringify({ input: att })
        )
      );
    }
  });
  ...
  if (rawEvent.organizer) {
    ...
    if (rawEvent.organizer.sentBy)
      event.organizer.sentBy = rawEvent.organizer.sentBy;
  }

  // Method
  if (
    rawEvent.method &&
    allowedMethods.indexOf(rawEvent.method.toUpperCase()) === -1
  ) {
    return new Error(
      "event.method must be one of the following: " + allowedMethods.join(", ")
    );
  }
  ...
  if (
    rawEvent.status &&
    allowedStatuses.indexOf(rawEvent.status.toUpperCase()) === -1
  ) {
    return new Error(
      "event.status must be one of the following: " + allowedStatuses.join(", ")
    );
  }
